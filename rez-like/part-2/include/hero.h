...
#define HERO_HURT        0.6
#define HERO_TUMBLE      1.4     // radians of roll at the peak of the tumble
#define HERO_KICK        0.5     // the legs kick this far, in mesh units, when hit
...
struct hero {
	...
	int held;                    // placed in the world, not in front of the eye
	...
	double thrust_at, hurt_at;
	double trail_due;
};

void hero_reset(struct hero *hero);
// the pilot is put at a place in the world, facing forward, and stays there
// while the eye moves around it. the launch does this, then lets go
void hero_hold(struct hero *hero, struct vec at, struct vec forward, struct vec right,
	       struct vec up);
void hero_release(struct hero *hero);
// follows the cursor and the camera. released and hurt are the events of
// this frame
