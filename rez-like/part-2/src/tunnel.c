...
static double signal_front = -1.0;

void tunnel_signal(double front)
{
	signal_front = front;
}
		 ...
		 double now, const struct palette *pal)
{
	...
	for (double t = first; t < t_eye + RING_AHEAD && t <= end;
		...
		// where the tunnel opens on the void the rings fade away
		double closed = 1.0 - rail_open(rail, t);
		if (closed <= 0.0)
			continue;
		struct light colour = light_scale(tunnel_colour(rail, t, pal), closed);
		...
		// the signal, a wave of white running away from the eye
		if (signal_front >= 0.0 && t <= signal_front && t > signal_front - SIGNAL_TAIL) {
			double white = SIGNAL_GAIN * (1.0 - (signal_front - t) / SIGNAL_TAIL);
			for (int i = 0; i < RING_SIDES; i++)
				draw_line(cam, ring[i], ring[(i + 1) % RING_SIDES],
