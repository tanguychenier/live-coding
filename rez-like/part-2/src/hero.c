...
static struct vec pose(const struct hero *hero, struct vec local, int index, double now)
{
	...
	double hurt = 1.0 - (now - hero->hurt_at) / HERO_HURT;
	if (hurt < 0.0) hurt = 0.0;
	if (index == HERO_FOOT_R || index == HERO_FOOT_L || index == HERO_KNEE_R
	    || index == HERO_KNEE_L)
		local.y += HERO_KICK * sin(hurt * M_PI) * (index % 2 ? 1.0 : -1.0);
...
static struct vec place(const struct hero *hero, struct vec local, double now)
{
	double hurt = 1.0 - (now - hero->hurt_at) / HERO_HURT;
	if (hurt < 0.0) hurt = 0.0;
	double roll = hero->bank + HERO_TUMBLE * sin(hurt * M_PI);
...
void hero_hold(struct hero *hero, struct vec at, struct vec forward, struct vec right,
	       struct vec up)
{
	hero->held = 1;
	hero->at = at;
...
void hero_release(struct hero *hero)
{
	hero->held = 0;
}

void hero_update(struct hero *hero, const struct camera *cam, double cursor_x,
		 double cursor_y, int released, int hurt, double elapsed,
		 double now)
{
	if (released)
		hero->thrust_at = now;
	...
	struct vec right = place(hero, pose(hero, mesh->point[HERO_HAND_R], HERO_HAND_R, now), now);
	struct vec left = place(hero, pose(hero, mesh->point[HERO_HAND_L], HERO_HAND_L, now), now);
	...
	while (hero->trail_due >= 1.0) {
		...
		struct vec at = place(hero, pose(hero, mesh->point[foot], foot, now), now);
...
void hero_draw(const struct hero *hero, const struct camera *cam, double now)
{
	...
	for (int i = 0; i < mesh->points; i++)
		placed[i] = place(hero, pose(hero, mesh->point[i], i, now), now);
	// the body flashes red when hit, and white-hot when a chain leaves
	double hurt = 1.0 - (now - hero->hurt_at) / HERO_HURT;
	double thrust = 1.0 - (now - hero->thrust_at) / HERO_THRUST;
	struct light body = LIGHT_HERO;
	if (hurt > 0.0)
		body = light_mix(body, LIGHT_HURT, hurt);
	if (thrust > 0.0)
