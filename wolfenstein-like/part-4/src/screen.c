...
void screen_read_keys(struct screen *screen, struct keys *keys)
{
	while (XPending(screen->display)) {
		...
		if (event.type == KeyPress || event.type == KeyRelease) {
			...
			// you choose your weapon, you do not put up with
			// it. the game falls back to the bar on an empty
			// magazine, which is right, but on its own the
			// player never learns he holds two things.
...
void screen_present(struct screen *screen)
{
	...
	if (visible > view_width)
		visible = view_width;
	// the second row is a copy of the first. we grew every pixel twice,
	// once per row, a million writes a frame. the bottom row is the top
	// row, so build it once and copy the block: memcpy is far faster.
	for (int y = 0; y < view_height; y++) {
		unsigned int *row = view + y * view_width;
		int line = screen->top + y * scale;
		...
		for (int copy = 1; copy < scale; copy++) {
			int twin = line + copy;
			if (twin >= screen->height)
				break;
			memcpy(screen->pixels + twin * screen->width + screen->left,
			       screen->pixels + line * screen->width + screen->left,
			       (size_t)visible * scale * sizeof(*screen->pixels));
		}
	}
