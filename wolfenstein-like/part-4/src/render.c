	...
	double warm, int dark, int kind)
{
	...
	for (; y < bottom; y++) {
		...
		if (lit < 0.0)
			lit = 0.0;
		view[y * view_width + x] = tinted(color, lit);
		tex_y += step;
	}
}
...
void render_floor_and_ceiling(const struct player *player)
{
	...
	for (int y = HORIZON + 1; y < view_height; y++) {
		...
		// light does not change from pixel to pixel, it changes
		// from cell to cell. asking for it a hundred thousand
		// times a frame cost half the game. sample every eight
		// pixels and slide between: the eye sees the same ramp.
		int glow_step = 8;
		double glow = base + lit_at(ground_x, ground_y);
		double glow_next = glow, glow_slope = 0.0;
		// and what lies on the floor is only asked at a cell
		// change: one whole tile carries the same thing
		int cell_x = -99999, cell_y = -99999, painted = -1;
		enum loot item = LOOT_NONE;
		unsigned int body = BADGE_BODY, edge = BADGE_EDGE;

		for (int x = 0; x < view_width; x++) {
			if (x % glow_step == 0) {
				glow = glow_next;
				double far_x = ground_x + step_x * glow_step;
				double far_y = ground_y + step_y * glow_step;
				glow_next = base + lit_at(far_x, far_y);
				glow_slope = (glow_next - glow) / glow_step;
			} else {
				glow += glow_slope;
			}
			double light = glow > 1.0 ? 1.0 : glow;

			int tex_x = (int)(ground_x * TEX_SIZE) & TEX_MASK;
			int tex_y = (int)(ground_y * TEX_SIZE) & TEX_MASK;
			int at = tex_y * TEX_SIZE + tex_x;

			if ((int)ground_x != cell_x || (int)ground_y != cell_y) {
				cell_x = (int)ground_x;
				cell_y = (int)ground_y;
				painted = stencil_at(cell_x, cell_y);
				// what can be picked up glows on its own,
				// in its own colour: amber for rounds,
				// green for a patch up. you know from
				// across the room whether it is worth it.
				item = loot_at(cell_x, cell_y);
				body = BADGE_BODY;
				edge = BADGE_EDGE;
				switch (item) {
				case LOOT_GUN:  body = GUN_BODY;  edge = GUN_EDGE;  break;
				case LOOT_AMMO: body = AMMO_BODY; edge = AMMO_EDGE; break;
				case LOOT_MED:  body = MED_BODY;  edge = MED_EDGE;  break;
				default: break;
				}
			}
			unsigned int ground = item != LOOT_NONE
				? loot_tile(tex_x, tex_y, body, edge)
				: painted >= 0 ? stencil_tile(tex_x, tex_y, painted)
				: floor_texture[at];
			view[y * view_width + x] =
				tinted(ground, item != LOOT_NONE ? 1.0 : light);
			view[(view_height - 1 - y) * view_width + x] =
				tinted(ceiling_texture[at], light * CEILING_PART);
...
// tall the wall is drawn: near is tall, far is short. the depth buffer
// holds one value per column: the walls fill it and the things read it.
double *wall_depth;
...
void render_walls(const struct player *player)
{
	...
	for (int x = 0; x < view_width; x++) {
		...
		// keep it: this is what tells the things whether they
		// stand in front of that wall or behind it
		if (wall_depth)
			wall_depth[x] = hit.distance;
