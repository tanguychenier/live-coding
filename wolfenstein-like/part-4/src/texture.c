...
// three channels, sixty-four levels, two hundred and fifty-six values:
// forty-eight kilobytes, laid down once and for all at startup
static unsigned char light_table[3][LIGHT_LEVELS][CHANNEL + 1];

void make_light_table(void)
{
	for (int level = 0; level < LIGHT_LEVELS; level++) {
		double lamp = (double)level / (LIGHT_LEVELS - 1);
		for (int channel = 0; channel < 3; channel++) {
			int d = channel * GREEN_SHIFT;
			double part = lamp * ((LIGHT_LAMP >> d) & CHANNEL)
				+ GLOOM * ((LIGHT_NIGHT >> d) & CHANNEL);
			for (int v = 0; v <= CHANNEL; v++) {
				double out = v * part * (1.0 / CHANNEL);
				light_table[channel][level][v] =
					(unsigned char)(out > CHANNEL ? CHANNEL : out);
			}
		}
	}
}
...
{
	int level = (int)(lamp * (LIGHT_LEVELS - 1) + 0.5);
	if (level < 0)
		level = 0;
	if (level >= LIGHT_LEVELS)
		level = LIGHT_LEVELS - 1;
	return ((unsigned int)light_table[2][level][(color >> RED_SHIFT) & CHANNEL]
			<< RED_SHIFT)
		| ((unsigned int)light_table[1][level][(color >> GREEN_SHIFT) & CHANNEL]
			<< GREEN_SHIFT)
		| light_table[0][level][color & CHANNEL];
}
...
// the three colours of a tube, from the core to the edge
static const unsigned int NEON[3] = { NEON_CORE, NEON_TUBE, NEON_EDGE };
static const unsigned int ALARM[3] = { ALARM_CORE, ALARM_TUBE, ALARM_EDGE };
static const unsigned int COLD[3] = { COLD_CORE, COLD_TUBE, COLD_EDGE };
...
static void neon_strip(unsigned int *out, int cx, int cy, int half_w,
	int half_h, const unsigned int *glow)
{
	for (int y = 0; y < TEX_SIZE; y++)
		for (int x = 0; x < TEX_SIZE; x++) {
			...
			if (dx <= half_w && dy <= half_h) {
				...
				out[y * TEX_SIZE + x] = edge < 0.45 ? glow[0]
					: edge < 0.8 ? glow[1] : glow[2];
			}
		}
...
void make_ceiling_texture(void)
{
	...
	neon_strip(ceiling_texture, TEX_SIZE / 2, TEX_SIZE / 2, 34, 5, NEON);
}
